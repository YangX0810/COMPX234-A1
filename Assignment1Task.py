...
class Assignment1:
    ...
    def startSimulation(self):
        ...
        for machine_id in range (1,self.NUM_MACHINES+1):
            machine_thread = machineThread(machine_id, self.print_list)
            self.mThreads.append(machine_thread)

        ## Create Printer threads
        for printer_id in range (1,self.NUM_PRINTERS+1):
            printer_thread = printerThread(printer_id, self.print_list)
        ...
        for machine_thread in self.mThreads:
            machine_thread.start()
            print(f"Start the machine {machine_thread.machineID} thread")

        # Start the printer thread
        for printer_thread in self.pThreads:
            printer_thread.start()
            print(f"Start the printer {printer_thread.printerID} thread")
        ...
        for printer_thread in self.pThreads:
            printer_thread.join()
        print("All printer threads finished.")
